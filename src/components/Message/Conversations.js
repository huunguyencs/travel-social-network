...
export default function Conversations() {
  ...
  const handleAddChat = user => {
    ...
    setSearch('');
    dispatch(
      addUser(auth, user, message, socket, id => {
      ...
      <Grid item md={3} sm={2} xs={2}>
        <div className={classes.message_conversations}>
          <div className={classes.message_header}>
            <Typography className={classes.message_header_title}>
              TIN NHẮN
            </Typography>
          ...
          <div className={classes.message_search}>
            <form
              className={classes.message_search_form}
                    ...
                    <ListItem
                      ...
                      onClick={
                        conversation.isGroup
                          ? () => handleChooseGroup(conversation._id)
                          : () => handleAddChat(conversation.members[0])
                      }
                    >
                      <ListItemAvatar>
                        <Avatar
                      ...
                      <ListItemText
                        ...
                        secondary={
                          conversation.latestMessage.text
                            ? conversation.latestMessage.text.length > 20
                              ? conversation.latestMessage.text.slice(0, 20) +
                                '---' +
                                timeAgo(
                      ...
                      <ListItemIcon>
                        {!conversation.latestMessage.seen && (
                          <FiberManualRecord style={{ color: '#a5dec8' }} />
                        )}
                      </ListItemIcon>
                    </ListItem>
                  ))}
