...
export default function AddLocationForm(props) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        dispatch(tourAction.addLocation({ location: nameRef.current.valueOf, cost: costRef.current.value, indexDate: props.indexDate }))
        props.handleClose();
    }
        ...
        <Paper className={[classes.paperContainer, classes.addFormContainer]}>
            ...
            >
                <Autocomplete
                    ...
                    renderInput={(params) => <TextField {...params} name="location" label="Địa điểm" variant="outlined" required inputRef={nameRef} />}
                />
                <TextField
