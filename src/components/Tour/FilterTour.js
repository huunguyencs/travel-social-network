...
export default function FilterTour(props) {
    ...
    const { costParent, setCostParent, textParent, setTextParent} = props;
                ...
                >
                    ...
                    <Collapse in={openCost} timeout="auto" unmountOnExit>
                        <div style={{ marginBlock: 10, color: "#A5DEC8"}}>
                            <Typography gutterBottom style={{fontWeight: 500, display: "flex", justifyContent: "center"}}>
                                    {/* {cost[0]}00.000VND ~ {cost[1]}00.000VND */}
                                    {cost[0] === 0 ? 'Tối thiểu' : (new Intl.NumberFormat().format(cost[0] * 100000) + ' VND')}  - {cost[1] === 100 ? 'Tối đa' : (new Intl.NumberFormat().format(cost[1] * 100000) + ' VND')}
                            </Typography>
